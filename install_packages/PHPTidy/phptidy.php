...
define('CONFIGFILE', "./.soulphptidy.php");
define('CACHEFILE', "./.soulphptidy.cache");
...
if ($command=="replace" || $command=="format") {
    if ($replaced) {
...
Commands:
  ...
  format   Format files without backup
...
function phptidy($source) {
    ...
    // elseif => elseif
    $source = preg_replace("/else +if/i", "elseif", $source);
